...
- もし良ければ、考えた結果をzoomのチャットに投稿してみてください〜
...
1. リストから先頭のデータを取り出す際は「0」を指定します〜！
1. 文字列の途中に変数のデータを表示させたい場合、format関数を使用します！

</br>

> 回答例は[こちら](https://github.com/pymee/studygroup/blob/feature/modify_doc/8th/answer/mail_create_2_after.py)になります
> どうしても分からない場合は回答例を参考にしてみてください！
...
- 繰り返しの処理を行うときはforを使用します、forのインデントの位置を気をつけてください！
- 複数回使用するデータは変数に格納して使用すると、プログラムの変更が楽になります！

</br>

> 回答例は[こちら](https://github.com/pymee/studygroup/blob/feature/modify_doc/8th/answer/mail_create_3_example.py)になります
> どうしても分からない場合は回答例を参考にしてみてください！
