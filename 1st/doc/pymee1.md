...
+ プログラムの実行までの流れ
    1. 処理内容を書いた「ファイル名.py」を作成
    1. 「python3 ファイル名.py」もしくは「py ファイル名.py」で実行
+ 型って？
    + 値をしまう箱の種類
...
---
# プログラムの実行
7. 下記コマンドをコンソールもしくはターミナルに入力しプログラムを実行
+ windowsの場合
```
py 02_variable.py
```
...
このままでは、神主さんが誰かがプログラムを実行するたびに出力するおみくじの中身を書き換えないといけないことに！
(運用でカバーというやつですね)
...
---
# プログラムの実行
5. 下記コマンドをコンソールもしくはターミナルに入力しプログラムを実行
+ windowsの場合
```
py 03_ransu_if.py
```
...
# 詳しく動作をみてみよう！
3. 下記の想定結果通りになっていることを確認(下記は実行例)
```
$ python3 03_ransu_if.py
3
あなたの名前を入力してください
>>pymee
pymeeさんの運勢は、吉 すこしよしです!
```
+ 想定結果
...
---
# プログラムの実行
6. 下記コマンドをコンソールもしくはターミナルに入力しプログラムを実行
+ windowsの場合
```
py 05_dict.py
```
...
# 復習！
+ ifって？
条件1に合う場合(真,True)は処理１を実行、条件1に合わない場合(偽,Flase)はelse後の処理2を実行する
```
if 条件1:
    処理1
else:
